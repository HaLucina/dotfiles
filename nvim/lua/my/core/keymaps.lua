...
--     3. Therefore, I created a wrapper function `dispatch` to shorten and simplify this.
-- 
-- ## Usage of `f.dispatch`
--     `f.dispatch(f.myfunc, Keys)` is used when `f.myfunc` needs arguments (`Keys`).
...
-----|-------------|--------------|-----------------------------------|-----------
kmap( { 'n'      }, '<leader>vn',  '<cmd>Explore<CR>',                 opts.TT, { desc = "Clear search highlights and redraw (cmd input)" })
kmap( { 'n'      }, '<leader>vs',  '<cmd>source %<CR>',                opts.TT, { desc = "Clear search highlights and redraw (cmd input)" })
...
kmap( { 'n', 'x' }, 'p',           f.dispatch(f.paste_force_lf, 'p'),  opts.TT, { desc = "nothing"})
kmap( { 'n', 'x' }, 'P',           f.dispatch(f.paste_force_lf, 'P'),  opts.TT, { desc = "nothing"})
...
kmap( { 'n'      }, '<A-d>',       '<C-d>zz',                          opts.TT, { desc = "Half page down and center" })
kmap( { 'n'      }, '<A-u>',       '<C-u>zz',                          opts.TT, { desc = "Half page up and center" })
kmap( { 'n'      }, '<leader>sv',  '<C-w>v',                           opts.TT, { desc = "Split window vertidispatchy" })
