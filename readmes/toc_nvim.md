...
path: [nvim/lua/my/lazy.lua](../nvim/lua/my/lazy.lua)
...
path: [lua/my/plugins/init.lua](lua/my/plugins/init.lua)
...
### smart-splits.lua
path: [nvim/lua/my/plugins/smart-splits.lua](nvim/lua/my/plugins/smart-splits.lua)

機能と目的:  
- Neovim内の分割画面へ移動できるようする。メインの目的は「Neovim <-> WezTerm」の画面移動を成立させる為。
...
### which-key
path: [lua/my/plugins/which-key.lua](lua/my/plugins/which-key.lua)
...
操作:  
- リーダーキー（`Space`）を押すと、500ミリ秒後にwhich-keyがアクティブになり、利用可能なオプションが表示される。


### telescope
path: [lua/my/plugins/telescope.lua](lua/my/plugins/telescope.lua)
...
-   目的: 構文ハイライト、インデント他の機能を提供します。構文ツリーにアクセスできます。
...
-   依存関係: `plenary`が必要です。
-   設定と操作:  `todo_comments.setup`を呼び出してプラグインを構成します。
-   キーマップ (ノーマルモード):
...
-   設定と操作:  `config`を`true`に設定すると、デフォルトで`lazy.nvim`がプラグインを要求し、`setup`関数を呼び出します。
-   操作:
